   ...
   Array.from(button).forEach(b => {
      b.addEventListener("click", (e) => {
         if (e.target.className === "add-product") {
            ...
            let priceElement = Number(currentElement.nextElementSibling.textContent);
            ...
            textArea.textContent += `Added ${brandElement} for ${priceElement.toFixed(2)} to the cart.\n`;
         } else {

            let totalPrice = Number(Object.values(list).reduce((a, b) => a + b, 0));
            textArea.textContent += `You bought ${Object.keys(list).join(', ')} for ${totalPrice.toFixed(2)}.`;
